    ...
    try:
        while res:  # runs over the retrieved entries
            ...
            if entry['standard_type'] in endpoints and entry["relation"] in relations:
                ...
                df = df.append({'smiles': entry["canonical_smiles"], 'standard_type': entry['standard_type'], 'value': entry["standard_value"], 'units': entry["standard_units"],
                                'chembl_id': entry["molecule_chembl_id"], 'document_chembl_id': entry["document_chembl_id"],
                                'data_validity_comment': entry["data_validity_comment"], 'data_validity_description': entry["data_validity_description"],
                                'activity_comment': entry["activity_comment"], 'warning_flag': warning_flag}, ignore_index=True)
