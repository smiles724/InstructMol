...
class ActivityCliffs:
    ...
    def find_cliffs(self, similarity: float = 0.9, potency_fold: float = 10, in_log10: bool = True, custom_cliff_function: Callable = None, mmp: bool = False):
...
def get_levenshtein_matrix(smiles: List[str], normalize: bool = True):
    ...
    # https://towardsdatascience.com/text-similarity-w-levenshtein-distance-in-python-2f7478986e75#:~:text=Levenshtein%20distance%20is%20a%20lexical,make%20it%20like%20string%202.
    smi_len = len(smiles)
...
def get_tanimoto_matrix(smiles: List[str], radius: int = 2, nBits: int = 1024):
    ...
    # Fill in the lower triangle without having to loop (saves ~50% of time)
    m = m + m.T - np.diag(np.diag(m))
    np.fill_diagonal(m, 0)   # Fill the diagonal with 0's
    return m
